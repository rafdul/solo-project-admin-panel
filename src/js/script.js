...
document.querySelector('form').addEventListener('submit', function(event) {
  ...
  if(nameInput.value === '') {
    ...
    nameInput.parentElement.querySelector('.error').innerHTML = 'Pole "Name" jest wymagane';
  }
  ...
  if(regTel.test(telAddressInput.value) == false) {
    isFormValidate = false;
    telAddressInput.parentElement.querySelector('.error').innerHTML = 'Błędny numer telefonu';
  }
  ...
  if(regPassword.test(passwordInput.value) == false) {
    isFormValidate = false;
    passwordInput.parentElement.querySelector('.error').innerHTML = 'Hasło musi mieć min. 5 znaków';
  }
  if (repeatInput.value != passwordInput.value) {
    isFormValidate = false;
    repeatInput.parentElement.querySelector('.error').innerHTML = 'Powtórz poprawne hasło';
  }
